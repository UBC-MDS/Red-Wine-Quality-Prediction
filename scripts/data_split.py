...
def main(input_file, output_x_train, output_x_test, output_y_train, output_y_test, test_size, random_state):
    ...
    X_train.to_csv(f'result/tables{output_x_train}', index=False)
    X_test.to_csv(f'result/tables{output_x_test}', index=False)
    y_train.to_csv(f'result/tables/{output_y_train}', index=False)
    y_test.to_csv(f'result/tables/{output_y_test}', index=False)
